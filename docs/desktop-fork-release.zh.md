...
| 已下载更新、弹窗反复出现但版本不变 | kb3/kb4 及更早版本 Squirrel/ShipIt 不可靠；需 **kb5+**（退出后用 `update.zip` 安装）或 **手动 DMG** |
| 自动更新下载完成但 Restart now 无效 | 同上；关窗口不等于退出，需 **Cmd+Q** 或点 Restart now；未签名包推荐 **手动 DMG** |
...
`https://github.com/wanwan2qq/multica/releases/download/v0.4.38-kb4/multica-desktop-0.4.38-kb4-mac-arm64.dmg`

**kb5+** 起 macOS 自动更新在退出时用缓存的 `update.zip` 覆盖 `/Applications/Multica.app`（绕过 ShipIt）。应用须安装在 `/Applications/Multica.app`。

...
- `--sharp-wasm`（或 `.env` / 环境变量 `SHARP_WASM=1`）：旧 KVM / 无 SSE4.2 的 CPU 上，新版 `sharp` 预编译包要求 x86-64-v2，会导致 `next build` 处理 PNG 失败；打开后会**固定 sharp@0.32.6**（仍支持 v1）。**默认关闭**，现代 CPU / CI 不必开。标志名保留 `SHARP_WASM` 以兼容已有 `.env`。
...
| `sharp` / `Unsupported CPU` 且路径里是 `sharp@0.35.x` | Next 仍在用自带的新 sharp。需含 **pnpm overrides 强制 0.32.6** 的补丁包（勿只用根依赖 `pnpm add sharp@0.32.6`） |

 ...
 * 2. Wiki-style knowledge references [[path]] → [path](kb:encoded-path)
 * 3. (readonly only) Bare issue identifiers MUL-123 → [MUL-123](mention://issue/MUL-123)
...
export function preprocessMarkdown(
  ...
  const step2 = preprocessKnowledgeLinks(step1);
  const step3 = opts?.autolinkIssueIdentifiers
    ? preprocessIssueIdentifiers(step2)
    : step2;

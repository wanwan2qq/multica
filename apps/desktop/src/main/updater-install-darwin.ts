...
export function darwinUpdateZipPath(homeDir: string): string {
  return join(darwinUpdaterCacheDir(homeDir), "update.zip");
}
...
export function scheduleDarwinPostQuitInstall(options: {
  ...
  if (!existsSync(updateZip)) return false;
...
ditto "$STAGING/Multica.app" "$APP"
open "$APP"
`;
  writeFileSync(scriptPath, script, "utf8");
  chmodSync(scriptPath, 0o755);

  const child = spawn(scriptPath, [String(options.pid), updateZip, appBundle], {
    detached: true,
    stdio: "ignore",
  });
  child.unref();

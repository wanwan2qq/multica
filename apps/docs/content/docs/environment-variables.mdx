...
| Knowledge browse | `KNOWLEDGE_GIT_TOKEN` | PAT for private Gitea / Forgejo / GitLab knowledge repos (`read_api` + `read_repository` on GitLab) |
| Knowledge browse | `KNOWLEDGE_GIT_PROVIDER` | Optional override; set `gitlab` for two-segment GitLab URLs. Nested `group/sub/project` paths are detected automatically |
| Knowledge browse | `GITHUB_TOKEN` | Optional GitHub token when the knowledge repo is a private GitHub repository |

...
The feature is intended for internal and self-hosted dogfood. Both `plugins_v1` and `private_plugins_v1` must be enabled. If either flag is off, private discovery, upload, and mutation APIs fail closed. Already pinned Runs remain reproducible.

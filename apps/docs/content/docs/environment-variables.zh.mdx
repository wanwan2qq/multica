...
| 知识库浏览 | `KNOWLEDGE_GIT_TOKEN` | 私有 Gitea / Forgejo / GitLab 知识库的 PAT（GitLab 需 `read_api` + `read_repository`） |
| 知识库浏览 | `KNOWLEDGE_GIT_PROVIDER` | 可选覆盖；两段路径的 GitLab 设为 `gitlab`。嵌套 `group/sub/project` 会自动识别 |
| 知识库浏览 | `GITHUB_TOKEN` | 知识库是私有 GitHub 仓时可选 |
